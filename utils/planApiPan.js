const DEFAULT_PAN_VERIFY_URL = "https://planapi.in/Api/Ekyc/PanVerification";
...
const getPlanApiConfig = () => {
  ...
  if (!tokenId || !apiUserId || !apiPassword) {
    throw new Error("PAN verification service is not configured");
  }
  ...
  return {
    ...
    verifyUrl: process.env.PLANAPI_PAN_VERIFY_URL?.trim() || DEFAULT_PAN_VERIFY_URL,
  };
};
...
const verifyPanWithPlanApi = async (panNumber) => {
  const config = getPlanApiConfig();
  const testMode = config.apiMode !== "1";

  const body = new URLSearchParams({
    Panid: panNumber.trim().toUpperCase(),
  ...
  return {
    message: message || "PAN Card verified successfully",
    providerResponse: payload || raw,
    registeredName: payload?.response?.registered_name || null,
    fatherName: payload?.response?.father_name || null,
    panType: payload?.response?.pan_type || null,
    testMode,
    apiMode: config.apiMode,
  };
};

export { verifyPanWithPlanApi };
